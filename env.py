...
class R2RPanoBatch():
    ...
    def __init__(self, opts, features, img_spec, batch_size=64, seed=10, splits=['train'], tokenizer=None):
        ...
        for i, item in enumerate(json_data):
            ...
            if not is_experiment(): break  #  if this is in developing mode, load only a small amount of data
            ...
            for j, instr in enumerate(item['instructions']):
                ...
                if tokenizer:
                    ...
                    if opts.similarity:
                        if 'sub_encoding' not in item:  # we may already include 'instr_encoding' when generating synthetic instructions
                            subs=[]
                            for sub in item['subs']:
                                subs+=sub.split('~')
                            new_item['sub_encoding']=[]
                            for sub in subs:
                                new_item['sub_encoding'].append(tokenizer.encode_sentence(sub))
                            new_item['sub_cnt']=len(subs)

                        else:
                            new_item['sub_encoding'] = item['sub_encoding']
    ...
    def _shortest_path_action(self, state, goalViewpointId):
        ...
        target_rel = self.graphs[state.scanId].node[nextViewpointId]['position'] - [state.location.x, state.location.y,state.location.z]
    ...
    def _pano_navigable(self, state, goalViewpointId):
        ...
        for viewpoint_id, weight in navigable_graph.items():
            ...
            dict_tmp['position'] = node['position']
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            if 'sub_encoding' in item:
                obs[-1]['sub_encoding'] = item['sub_encoding']
                obs[-1]['subs'] = item['subs']
                obs[-1]['sub_cnt'] = item['sub_cnt']
        return obs
