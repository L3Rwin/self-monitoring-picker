...
from utils import setup, read_vocab, Tokenizer, set_tb_logger, is_experiment, padding_idx, resume_training, save_checkpoint,EarlyStopping
...
parser.add_argument('--exp_name_secondary', default='', type=str,
                    help='name of the experiment. \
                        It decides where to store samples and models')
parser.add_argument('--ft_suffix', default='ft', type=str,
                    help='name of the experiment. \
                        It decides where to store samples and models')
...
parser.add_argument('--encoding_length', default=80, type=int, help='maximum length of captions')
# Evaluation options
...
parser.add_argument('--early_stopping',
                    default=1, type=int, help='whether to early stop')

parser.add_argument('--similarity',
                    default=0, type=int, help='whether to use similarity')
parser.add_argument('--sub_ft', default=0, type=int,
                    help='fine tune sub instruction model')
parser.add_argument('--freeze_pm_ft', default=0, type=int,
                    help='fine tune sub instruction model freezing PM')
...
def main(opts):
    ...
    tok = Tokenizer(opts.remove_punctuation == 1, opts.reversed == 1, vocab=vocab, encoding_length=opts.encoding_length)
    ...
    # create language instruction encoder for subs
    print('Using {} as sub encoder ...'.format(opts.lang_embed))
    if 'lstm' in opts.lang_embed:
        sub_encoder = EncoderRNN(**encoder_kwargs)
    else:
        raise ValueError('Unknown {} language embedding'.format(opts.lang_embed))
    print(sub_encoder)
    ...
    if opts.resume=='':
        print("restart finetune")

        params =list(encoder.parameters()) + list(model.parameters())
        optimizer = torch.optim.Adam(params, lr=opts.learning_rate)

        model, encoder, optimizer, best_success_rate, sub_encoder = resume_training(opts, model, encoder, optimizer,
                                                                                    sub_encoder)
        optimizer.add_param_group({'params': list(sub_encoder.parameters()), 'lr': opts.learning_rate})

    else:
        print("resume finetune")
        params = list(encoder.parameters()) + list(model.parameters()) + list(sub_encoder.parameters())
        optimizer = torch.optim.Adam(params, lr=opts.learning_rate)
    ...
    print("initialize best SR to:", best_success_rate)
    ...
    save_checkpoint({
        ...
        'optimizer': optimizer.state_dict(),
        'max_episode_len': opts.max_episode_len,
    }, 1, checkpoint_dir=opts.checkpoint_dir, name=opts.ft_suffix+"_FINAL")
